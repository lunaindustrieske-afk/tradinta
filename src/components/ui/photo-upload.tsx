...
const PhotoUpload = React.forwardRef<HTMLDivElement, PhotoUploadProps>(
  ({ onUpload, onLoadingChange, initialUrl, label, className, ...props }, ref) => {
    ...
    const onDrop = React.useCallback(
      ...
      [] // The handleUpload function is now defined inside so it has access to the latest onUpload
    );
    ...
    return (
      <div ref={ref} className={cn('space-y-2', className)} {...props}>
         <Label>{label}</Label>
        {preview ? (
          <div className="relative group aspect-square w-full rounded-md border-2 border-dashed flex items-center justify-center cursor-pointer hover:bg-muted">
             <Image
                src={preview}
                alt="Preview"
                fill
